  ...
  module Serializer
    ...
    module Core
      ...
      def record_includes(record, items, known, fieldsets, params)
        ...
        items.each_with_object([]) do |(item, item_includes), included|
          ...
          Array(rel_objects).each do |rel_obj|
            ...
            rel_obj_id = item_serializer.record_type.to_s +
      ...
      def relationship_ids(relationship, record, params)
        ...
        if has_many = relationship[:relationship_type] == :has_many
          ids_rails_postfix = '_ids'
        end

        if serializer = rel_opts[:serializer] && serializer.is_a?(Class)
          unless ids_meth = rel_opts[:ids_method_name]
            ids_meth = relationship[:name].to_s + ids_rails_postfix
          end

          ids = record.public_send(ids_meth) if record.respond_to?(ids_meth)

          return id_hash(ids) unless ids.nil? && has_many

          return ids.map! { |oid| id_hash(oid) } unless ids.nil?
        end
        ...
        ids = rel_objects.map do |rel_obj|
          rel_obj_ser = serializer

          if rel_obj_ser.is_a?(Proc)
            rel_obj_ser = call_proc(rel_obj_ser, rel_obj, params)
          end

          rel_obj_ser ||= ::JSONAPI::Serializer.for_object(rel_obj)
          rel_obj_ser.id_hash(rel_obj_ser.id_from_record(rel_obj, params))
        end
